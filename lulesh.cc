...
Main::Main(CkArgMsg *msg) {
  ...
  CkPrintf("Constructed chares\n");

  /////
  insituChares = CProxy_InSitu::ckNew(chareDimX*chareDimY*chareDimZ);
  statusChares = BabelFlow::charm::CProxy_StatusMgr::ckNew(0, 1);
  /////
}
...
Domain::pup(PUP::er &p) {
  ...
  if(p.isUnpacking()) {
    ...
    std::string st_data;
    p|st_data;
    // CkPrintf("Domain unPUP, ascent node: %s\n", st_data.c_str());
    p|st_data;
    m_ascent_node.parse(st_data);
  }
  else {
    std::string st_data = m_ascent_node.to_string();
    // CkPrintf("Domain PUP, ascent node: %s\n", m_ascent_node.to_string().c_str());
    p|st_data;
  }
  // Simulation Related 
  p|domain->deltaTime;
...
Domain::resumeElemIteration() {
  ...
  if(iterations == 100)
  {
    // double minv = domain->m_p[0];
    // double maxv = domain->m_p[0];
    // for(int i = 0; i < numElems; ++i)
    // {
    //   minv = std::min(minv, domain->m_p[i]);
    //   maxv = std::max(maxv, domain->m_p[i]);
    // }
    int32_t chare_idx = thisIndex.z*chareDimY*chareDimX + thisIndex.y*chareDimX + thisIndex.x;
    // CkPrintf("Chare [%d]: minv = %f  maxv = %f\n", chare_idx, minv, maxv);

    CharmAscentData ascent_data(m_ascent_node);
    // assert(ascent_data.m_cond_node["state/domain_id"].as_int32() == chare_idx);
    int dom_id = ascent_data.m_condNode["state/domain_id"].as_int32();
    CkPrintf("chare_idx = %d   dom_id = %d\n", chare_idx, dom_id);
    // CkPrintf("dom node: %s\n", m_ascent_node.to_string().c_str());
    ///
    int status_ep = BabelFlow::charm::CkIndex_StatusMgr::status(NULL);
    CkArrayID aid = statusChares;
    int status_id = CkGroupID(aid).idx;

    // if(chare_idx == 0)
    //   statusChares[0].newCycle(333);
    // return; 
    ///
    insituChares[chare_idx].CreateInSituChares(ascent_data, status_ep, status_id);
  }
}
